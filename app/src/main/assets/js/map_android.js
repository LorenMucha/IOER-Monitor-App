...
function init_map(){
	...
	var subMenu_html = '<div class="subMenu" id="subMenuPoint1">'+
							...
							'<div class="subMenu" id="subMenuPoint3">'+
								'<div id="content_three">'+
									'<div id="slider_label">2014</div>'+
									'<div type="range" id="time_slider"></div>'+
								'</div>'+
							'</div>'+
	...
	// set the variables of the xml doc of the ajax response
	...
	$("#slider_label").text(maxValueInArray);
	...
	// Funktion kann leider nicht ausgelagert werden
	L.control.scale({
		imperial:false,
	...
	$(function create_layers() {
		...
		var wms_layer;
		var wms = new L.tileLayer.wms(xData, {
		...
		var ebenent = L.control.layers(baseMaps, timelayers, {collapsed: false});
		ebenent._map = map;
		...
		$(function time_slider() {
			...
			$("#time_slider").slider({
				...
				slide: function (event, ui) {
					//var time = ui.value;
					$("#slider_label").text(years[ui.value]);
					map.removeLayer(wms_layer);
					wms_layer = wms;
					legende_link = slider_legende;
					wms_layer.setParams({layers: layer_name + years[ui.value] + '_a'});
					wms_layer.addTo(map);
					console.log("Add Time ==============" + layer_name + years[ui.value] + '_a');
	...
	$(function slider() {
		if (localStorage.getItem("slider") == "true"){
			window.open("side/android_slider.html","_self");
			localStorage.setItem("slider",true);
		}
		else{
